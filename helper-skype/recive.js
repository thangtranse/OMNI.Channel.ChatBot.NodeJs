...
const handleMessage = async (_data) => {
    ...
    if (checkDataUser) {
        ...
        if (infoRoomRocket.success) {
            ...
            let createWebhookRocket = apiRocket.createOutGoingWebhookRocket(config.url_webhook.URL_WEBHOOK_CALLBACK_SKYPE, nameRoomRocket).then(data => data).catch(data => data);
